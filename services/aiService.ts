...
const evaluateWindowAdvanced = (window: CellState[], player: Player) => {
  ...
  if (count === 5) {
    ...
    windowScore =
      openEnds === 2
        ? PATTERN_SCORES.LIVE_FOUR
        : openEnds === 1
        ? PATTERN_SCORES.DEAD_FOUR
        : 0;
  } else if (count === 3) {
    windowScore =
      openEnds === 2
        ? PATTERN_SCORES.LIVE_THREE
        : openEnds === 1
        ? PATTERN_SCORES.DEAD_THREE
        : 0;
  } else if (count === 2) {
    windowScore =
      openEnds === 2
        ? PATTERN_SCORES.LIVE_TWO
        : openEnds === 1
        ? PATTERN_SCORES.DEAD_TWO
        : 0;
  } else if (count === 1) {
    windowScore =
      openEnds === 2
        ? PATTERN_SCORES.LIVE_ONE
        : openEnds === 1
        ? PATTERN_SCORES.DEAD_ONE
        : 0;
  }
...
export const scorePositionAdvanced = (board: BoardState, player: Player) => {
  ...
  for (let r = 0; r < BOARD_SIZE; r++) {
    for (let c = 0; c < BOARD_SIZE; c++) {
      ...
      if (
        r <= BOARD_SIZE - WINNING_LENGTH &&
        c <= BOARD_SIZE - WINNING_LENGTH
      ) {
        const window5: CellState[] = [];
        for (let i = 0; i < WINNING_LENGTH; i++)
          window5.push(board[r + i][c + i]);
        score += evaluateWindowAdvanced(window5, player);
      }
...
export const analyzeThreats = (
  board: BoardState,
  player: Player,
  scoreFunction: (b: BoardState, p: Player) => number
) => {
  const opponent = player === 'X' ? 'O' : 'X';
  const emptyCells = getValidMoves(board);

  let bestOpponentMove: {
    move: { row: number; col: number };
    score: number;
  } | null = null;
  let maxOpponentScore = -Infinity;

  let bestAiMove: { move: { row: number; col: number }; score: number } | null =
    null;
    ...
    for (let i = -4; i <= 0; i++) {
      ...
      if (isValidWindow) {
        const playerCount = windowCells.filter(
          (cell) => cell.state === player
        ).length;
        const opponentCount = windowCells.filter(
          (cell) => cell.state !== null && cell.state !== player
        ).length;
...
const findBestMoveWithMinimax = (
  ...
  if (skillLevel === 'easy') {
    const { bestOpponentMove, bestAiMove } = analyzeThreats(
      board,
      aiPlayer,
      scoreFunction
    );
    ...
    if (
      bestOpponentMove &&
      bestOpponentMove.score >= PATTERN_SCORES.DEAD_THREE
    ) {
      ...
      if (bestAiMove && bestAiMove.score > bestOpponentMove.score * 1.2) {
        console.log(
          'Easy AI: Found a better offensive move, ignoring block.',
          bestAiMove.move
        );
        return bestAiMove.move;
      }
      console.log(
        "Easy AI: Blocking opponent's 3-in-a-row threat.",
  ...
  if (skillLevel === 'medium' || skillLevel === 'hard') {
    ...
    if (bestOpponentMove && bestOpponentMove.score >= threatThreshold) {
      ...
      if (!bestAiMove || bestOpponentMove.score > bestAiMove.score) {
        console.log(
          `AI (${skillLevel}): Blocking critical player threat.`,
          bestOpponentMove.move
        );
        return bestOpponentMove.move;
      }
  ...
  console.error(
    'Minimax failed to return a valid move, picking a random one from available moves.'
  );
  return emptyCells[Math.floor(Math.random() * emptyCells.length)];
};

// Asynchronous, non-blocking recursive version for the 'hard' AI to prevent UI freezing
const minimaxAsync = async (
  board: BoardState,
  depth: number,
  alpha: number,
  beta: number,
  ...
  movesToConsider?: { row: number; col: number }[]
): Promise<{ score: number; move?: { row: number; col: number } }> => {
  if (depth === 0 || checkWin(board, 'X') || checkWin(board, 'O')) {
    return { score: scoreFunction(board, aiPlayer) };
  }
  ...
  if (maximizingPlayer) {
    ...
    for (const move of possibleMoves) {
      nodeCounter.count++;
      if (nodeCounter.count % 25 === 0) {
        // Yield to event loop every 25 nodes
        await new Promise((resolve) => setTimeout(resolve, 0));
      }
      const newBoard = board.map((r) => [...r]);
      newBoard[move.row][move.col] = aiPlayer;
      const result = await minimaxAsync(
        newBoard,
        depth - 1,
        alpha,
        beta,
...
const findBestMoveWithMinimaxAsync = async (
  board: BoardState,
  aiPlayer: Player,
  depth: number,
  scoreFunction: (b: BoardState, p: Player) => number
  ...
  if (bestOpponentMove && bestOpponentMove.score >= threatThreshold) {
    if (!bestAiMove || bestOpponentMove.score > bestAiMove.score) {
      console.log(
        `AI (hard): Blocking critical player threat via tactical analysis.`,
        bestOpponentMove.move
      );
      return bestOpponentMove.move;
    }
  ...
  console.error(
    'Async Minimax failed to return a valid move, picking first available.'
  );
  return emptyCells[0];
};
...
export const getAIMove = async (
  ...
  console.log(
    `Local AI: Minimax (depth ${depth}) for ${skillLevel} opponent. (Opening book miss)`
  );
  ...
  if (onThinking) {
    ...
    const probableMove = findBestMoveWithMinimax(
      board,
      aiPlayer,
      1,
      scorePositionAdvanced,
      skillLevel
    );
  ...
  if (skillLevel === 'hard') {
    return findBestMoveWithMinimaxAsync(
      board,
      aiPlayer,
      depth,
      scorePositionAdvanced
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      const move = findBestMoveWithMinimax(
        board,
        aiPlayer,
        depth,
        scorePositionAdvanced,
