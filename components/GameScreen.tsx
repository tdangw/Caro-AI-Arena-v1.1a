...
const GameScreen: React.FC<GameScreenProps> = ({ gameMode, bot, onlineGameId, onExit, theme, pieces, playerInfo, activeEffect, activeVictoryEffect, activeBoomEffect, activeBoard, isPaused, onOpenShop, onOpenInventory }) => {
    ...
    useEffect(() => {
        ...
        if (isGameOver && winner && !isGameResultProcessedRef.current && user) {
            ...
            const duration = isPVE ? INITIAL_GAME_TIME - pveLogic.totalGameTime : undefined;
        ...
        <div className="w-full max-w-[75vmin] mx-auto relative z-10 flex flex-col justify-center">
            ...
            <main className="flex-grow flex flex-col justify-center relative">
                 ...
                 <div className="flex justify-between items-end px-2 mb-[4px] -mt-px">
                    <PlayerInfo ref={playerAvatarRef} name={playerInfo.name} avatar={playerInfo.avatar.url} level={playerInfo.level} align="left" player={playerMark} isCurrent={currentPlayer === playerMark} piece={allPieces[playerMark]} cp={gameState.cp} />
                    ...
                    <PlayerInfo ref={opponentAvatarRef} name={opponentInfo.name} avatar={opponentInfo.avatar} level={opponentInfo.level} align="right" player={playerMark === 'X' ? 'O' : 'X'} isCurrent={currentPlayer !== playerMark} piece={allPieces[playerMark === 'X' ? 'O' : 'X']} cp={opponentInfo.cp} />
                </div>
